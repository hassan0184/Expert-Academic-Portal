...
def users(request):
   if request.POST:
       ...
       allposts = Demo.objects.filter(completename__icontains=query).exclude(completename= 'admin' and 'Guest')
       return render(request, 'users.html',{'regestered_users':allposts,'admin':1})
   else:
     val=request.GET.get('vl')
     if val == 'aes':
         
         data = Demo.objects.exclude(completename=  'admin' and 'Guest').order_by('Name')
         return render(request, 'users.html',{'regestered_users':data,'admin':1})
     elif val == 'des':
         data = Demo.objects.exclude(completename=  'admin' and 'Guest').order_by('-Name')
         return render(request, 'users.html',{'regestered_users':data,'admin':1})
     else:       
      data = Demo.objects.exclude(completename=  'admin' and 'Guest')
      return render(request, 'users.html',{'regestered_users':data,'admin':1})
@csrf_exempt
