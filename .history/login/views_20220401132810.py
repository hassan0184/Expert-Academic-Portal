...
def adminprofile(request):
    ...
    if request.POST:
     ...
     obj.save()
            ...
            labels_graph = []
            data_graph = []
            hot_trends = TrendsTopic.objects.all().order_by('-count')[:4]
            for i in hot_trends:
             labels_graph.append(i.query_type)
             data_graph.append(i.count)
            ...
            user_count=Demo.objects.all().count()
            topic_count=TrendsTopic.objects.all().count()
       
            return render(request, 'adminprofile.html', {'dataa': fetch_data,'data': fetch_data,'user_data':data, 'ID': userid,'comment': comment,'COUNT':comment_count,'REPLY':replycomment,'admin':1 ,'labels_graph': labels_graph,'data_graph': data_graph,'Total_Queries_Count':query_count,'Total_Users_Count':user_count,'Total_Tags_Count':topic_count})
    ...
    if request.session.has_key('is_logged'):
            if request.POST:
               ...
               if len(query) == 0:
                    ...
                    return render(request, 'home.html', {'dataa': fetch_data,'data': fetch_data,'user_data':data, 'ID': userid,'comment': comment,'COUNT':comment_count,'REPLY':replycomment,'currentid':data,'follow_data':follow_data,'follower_data':followers_data,'follow_data_count':follow_data_count,'trends':hot_trends,'guest':1 })

               return render(request, 'home.html', {'dataa': fetch_data,'data': allposts,'user_data':data, 'ID': userid,'comment': comment,'COUNT':comment_count,'REPLY':replycomment,'currentid':data,'follow_data':follow_data,'follower_data':followers_data,"t":1 ,'follow_data_count':follow_data_count,'trends':hot_trends,'guest':1})
...
def users(request):
   if request.POST:
       query = request.POST['search_user']
       allposts = Demo.objects.filter(completename__icontains=query).exclude(completename= 'admin')
       return render(request, 'users.html',{'regestered_users':allposts,'admin':1})
   else:
     val=request.GET.get('vl')
     if val == 'aes':
         
         data = Demo.objects.exclude(completename= 'admin').order_by('Name')
         return render(request, 'users.html',{'regestered_users':data,'admin':1})
     elif val == 'des':
         data = Demo.objects.exclude(completename= 'admin').order_by('-Name')
         return render(request, 'users.html',{'regestered_users':data,'admin':1})
     else:       
      data = Demo.objects.exclude(completename= 'admin')
      return render(request, 'users.html',{'regestered_users':data,'admin':1})
@csrf_exempt
def deleteuser(request):
    id=request.POST.get('post_id')
    Demo.objects.filter(id=id).delete()
    return redirect('home')
